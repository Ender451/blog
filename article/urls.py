...
app_name='article'
urlpatterns = [
   #url(r'^$', views.articlelist, name='articlelist'),                                                 # 已修改为使用分页-不能再使用
   url(r'^$', views.ArticlelistView.as_view(), name='articlelist'),                                    #ok 与home index 一致  改为 基于类的 通用视图
   #url(r'(?P<year>[0-9]{4})/(?P<month>[0-9]{1,2})/$', views.archives, name='archives'),               # 已修改为使用分页-不能再使用
   url(r'(?P<year>[0-9]{4})/(?P<month>[0-9]{1,2})/$', views.Archivesview.as_view(), name='archives'),  # 改为 基于类的 通用视图 archives 归档文章列表
   #url(r'^category/(?P<pk>\d+)/$', views.category, name='category'),                                  # 已修改为使用分页-不能再使用
